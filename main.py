...
class TradingBotApplication:
    ...
    async def initialize(self):
        ...
        try:
            ...
            # Log enabled chains from the new config model
            chains_config = self.config_manager.get_chains_config()
            self.logger.info("✅ Chain configuration loaded:")
            self.logger.info(f"  Enabled chains: {chains_config.enabled_chains}")

            # Initialize trading engine
            self.logger.info("Initializing trading engine...")

            # The TradingBotEngine now expects the ConfigManager instance directly
