...
Stop Trading Bot, Keep Database Running
# Step 1: Stop ONLY the trading-bot service (postgres & redis stay running)
docker-compose stop trading-bot

# Verify: You should see postgres and redis still running
docker ps
# Expected output:
#   trading-postgres   ... Up
#   trading-redis      ... Up
#   (trading-bot should NOT appear)
...
# Get your credentials from secrets files
...
Option B: Run inside postgres container (if Python deps are available)

# This usually won't work because postgres container doesn't have Python
# but listing for completeness
docker exec -it trading-postgres ...
...
docker-compose stop trading-bot	Stops bot, keeps DB running
docker-compose up -d trading-bot	Starts bot back up
