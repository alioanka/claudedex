#!/usr/bin/env python3
"""REGIME_ALLOCATOR module — entry point.

Volatility-regime-aware capital allocator (ADVISORY ONLY). On a slow cadence
it classifies the current market regime from free BTC/ETH price data
(realized-vol expansion vs compression x trend vs range) and writes
regime-conditioned per-module capital-weight PROPOSALS to
regime_allocation_proposals for operator approval. It never trades, never
flips a live flag, never touches the killswitch or any pause flag.

Launched by main.py when REGIME_ALLOCATOR_MODULE_ENABLED=true (default false).
Health server: port REGIME_ALLOCATOR_HEALTH_PORT (default 8091).
"""
import asyncio
import logging
import os
...
log_dir = Path("logs/regime_allocator")
log_dir.mkdir(parents=True, exist_ok=True)

_fmt = logging.Formatter('%(asctime)s - %(name)s - %(levelname)s - %(message)s')
logger = logging.getLogger("RegimeAllocatorModule")
logger.setLevel(logging.INFO)

main_h = RotatingFileHandler(log_dir / 'regime_allocator.log',
                             maxBytes=5 * 1024 * 1024, backupCount=3)
main_h.setFormatter(_fmt)
logger.addHandler(main_h)

err_h = RotatingFileHandler(log_dir / 'regime_allocator_errors.log',
...
engine_logger = logging.getLogger("regime_allocator")
engine_logger.setLevel(logging.INFO)
for h in logger.handlers:
    engine_logger.addHandler(h)


_LAST_SUMMARY: dict = {"regime": None, "confidence": None,
                       "proposals_written": 0, "mirrored": 0, "skipped": None}
...
async def _start_health_server(port: int) -> None:
    ...
    async def health(_req):
        return web.json_response({"status": "ok", "module": "regime_allocator"})
...
async def main() -> None:
    logger.info("🌡️ Regime Allocator Module Starting...")
    ...
    health_port = int(os.getenv('REGIME_ALLOCATOR_HEALTH_PORT', '8091'))
    await _start_health_server(health_port)

    from modules.regime_allocator.core.regime_engine import (
        load_regime_config, run_loop,
    )

    async def get_config():
        async with pool.acquire() as conn:
            cfg = await load_regime_config(conn)
        # env fallbacks so the subprocess starts before the config_type exists
        cfg.setdefault('tick_interval_seconds',
                       int(os.getenv('REGIME_ALLOCATOR_TICK_INTERVAL', '3600')))
        return cfg

    # Wrap run_tick to capture the latest summary for /status (fail-soft).
    import modules.regime_allocator.core.regime_engine as _eng
