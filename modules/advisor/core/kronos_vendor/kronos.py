...
# VENDORED for ClaudeDex advisor module. Upstream used
# ``sys.path.append("../"); from model.module import *`` which only resolves
# when executed from inside a clone of the Kronos repo. Vendored here as a
# package-relative import so it is importable from anywhere on PYTHONPATH and
# works fully offline (no `model` top-level package required).
