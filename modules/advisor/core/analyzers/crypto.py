...
class CryptoAnalyzer(BaseAnalyzer):
    ...
    async def _analyze_internal(self, symbol: str, horizon: Horizon) -> AdviceResult:
        ...
        entry_low, entry_high, target, stop = horizon_levels(
            signals, direction, horizon, config=self.config, price_decimals=8
        )
    ...
    def _fetch_and_compute(
        ...
        # ATR-14 as a fraction of close — the volatility unit for horizon-aware
        # levels (see analyzers/levels.py). Falls back to BB-width proxy if NaN.
...
def _atr_pct(high, low, close, period: int = 14) -> Optional[float]:
    """
    ATR-`period` as a fraction of the last close (the volatility unit consumed by
    levels.horizon_levels). Returns None on insufficient/NaN data so the levels
    module falls back to the Bollinger-width proxy.
    """
    try:
