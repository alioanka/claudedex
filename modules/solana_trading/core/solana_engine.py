    ...
    async def _get_wallet_balance(self, *, force: bool = False) -> float:
        ...
        try:
            ...
            if (now - self._wallet_balance_last_err_log) >= self._wallet_balance_err_log_interval_s:
                ...
                # wave-15 hygiene: fail-soft RPC balance errors are WARNING, not
                # ERROR (trading continues on cached balance). Escalate to ERROR
                # only if the streak is long enough to indicate a real outage.
                _bal_log = logger.error if self._wallet_balance_error_streak >= 50 else logger.warning
                _bal_log(
