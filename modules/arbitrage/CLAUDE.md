...
## What it does
Spatial (cross-DEX) and triangular EVM arbitrage with flash-loan funding (Aave V3).
Spatial is production-grade; triangular path is currently gated behind MB-05 atomic-receiver guard.

## Entry point
`modules/arbitrage/main_arbitrage.py` — launched as a subprocess by `main.py` orchestrator when `ARB_MODULE_ENABLED=true`.
Engines: `modules/arbitrage/arbitrage_engine.py` (spatial), `triangular_engine.py` (gated), `solana_engine.py` (cross-chain helper).

## Key config (DB-backed via `ConfigManager`)
- `flash_loan_amount` — flash-loan size in ETH-equivalent per leg
- `chain_config.tokens` / `routers` / `arb_pairs` — selected via `EVM_DEX_ROUTING` (MB-24 multi-chain wiring)
...
`core.risk_manager.RiskManager.validate_trade(token_in, amount)` called at
`modules/arbitrage/arbitrage_engine.py:1600-1604` (P1-06). Injected via `set_risk_manager()` at `:962-964`.
