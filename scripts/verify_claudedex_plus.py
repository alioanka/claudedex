...
#!/usr/bin/env python3
"""
ClaudeDex Verifier (PLUS) — improved path handling
- Parses Markdown docs with key methods/classes
- Parses ASCII tree in corrected_file_structure.md preserving full relative paths
- Resolves bare filenames by searching under --root when necessary
- Validates presence, async-ness, signatures, visibility, and wrappers
- Finds duplicates (in-file & cross-file)
- Emits Markdown and JSON reports; supports CI fail flag
"""
import argparse
import ast
import json
...
class ExpectedItem:
    ...
    wrapper_for: Optional[str] = None   # e.g., "core.engine.run"
    decorators: Optional[List[str]] = None

@dataclass
class FoundMethod:
    name: str
    qualified_name: str  # "module:function" or "Class.method"
    is_async: bool
    lineno: int
    scope: str           # 'module' | f'class:{ClassName}' | f'func:{Outer}'
    params: List[str]
    has_varargs: bool
    has_varkw: bool
    ...
    decorators: List[str]
    calls: List[str]     # callee simple names encountered in body
...
# Resolve path: if rel_path is a bare filename that doesn't exist at root,
# try to find a unique match under root recursively.
        ...
        if "class " in line:
            ...
            if cm:
                ...
                file_to_expected[current_file].append(ExpectedItem(
                    name=name, kind="class", visibility=visibility
                ))
            continue
            ...
            if fm:
                ...
                for f in flags:
                    ...
                    if fl == "public":
                        ...
                        is_async = True
                    elif fl.startswith("wrapper="):
                        wrapper_for = f.split("=", 1)[1].strip()
                ...
                file_to_expected[current_file].append(ExpectedItem(
                    ...
                    wrapper_for=wrapper_for, decorators=decorators
                ))
            continue
...
# -----------------------------
# Structure parser (ASCII tree aware)
# -----------------------------
...
def _parse_tree_block_lines(text: str) -> List[Tuple[int, str, bool]]:
    ...
    for raw in text.splitlines():
        ...
        try:
            ...
            continue
        # Count depth by groups like '│   ' or 4 spaces
        level = 0
        i = 0
        while i < len(prefix):
            if prefix[i:i+4] in ('│   ', '    '):
...
class _AstCollector(ast.NodeVisitor):
    ...
    def _visit_func(self, node: ast.AST, is_async: bool) -> Any:
        ...
        if self._class_stack:
            scope = f"class:{self._class_stack[-1]}"
            qname = f"{self._class_stack[-1]}.{name}"
        elif self._func_stack:
            scope = f"func:{self._func_stack[-1]}"
            qname = f"{self._func_stack[-1]}.{name}"
        else:
            scope = "module"
            qname = name
        ...
        self.methods.append(FoundMethod(
            name=name, qualified_name=qname, is_async=is_async,
            lineno=getattr(node, 'lineno', -1), scope=scope,
            params=params, has_varargs=has_varargs, has_varkw=has_varkw,
...
def collect_file_symbols(py_path: Path) -> Tuple[List[FoundMethod], List[str], List[str]]:
    ...
    return collector.methods, collector.classes, []
...
def _compare_signature(expected: ExpectedItem, found_variants: List[FoundMethod], file_path: str) -> List[str]:
    ...
    for fm in found_variants:
        ...
        if expected.returns is not None:
            fr = fm.returns or "None"
            er = expected.returns or "None"
            if fr != er:
...
def _check_visibility(expected: ExpectedItem, found_variants: List[FoundMethod], file_path: str) -> List[str]:
    issues: List[str] = []
    if expected.visibility is None:
        return issues
    req = expected.visibility
    ok = False
    for fm in found_variants:
        if fm.visibility == req:
            ok = True
        else:
            issues.append(f"{file_path}:{fm.lineno} {fm.qualified_name} visibility mismatch expected={req} found={fm.visibility}")
    if ok:
        return []
    return issues
...
def _check_wrapper(expected: ExpectedItem, found_variants: List[FoundMethod], file_path: str) -> List[str]:
    ...
    for fm in found_variants:
        ...
        if calls_target:
            ...
            issues.append(f"{file_path}:{fm.lineno} {fm.qualified_name} documented wrapper for '{expected.wrapper_for}' but no call to '{target_tail}' detected")
    if any_calls:
        return []
    return issues
...
def generate_reports(
    root: Path,
    expected_map: Dict[str, List[ExpectedItem]],
    all_files_to_check: Optional[List[str]] = None,
    include_cross_file_duplicates: bool = True,
) -> Tuple[Dict[str, FileReport], Dict[str, List[str]]]:
    ...
    for rel_path in all_files_to_check:
        # Resolve path: exact or unique match by filename under root
        ...
        if exists and fpath.suffix == ".py":
            found_methods, found_classes, parse_errors = collect_file_symbols(fpath)
        ...
        for exp in expected:
            ...
            if not variants:
                missing.append(f"def {exp.name}()")
                continue

            if exp.is_async is not None:
                if not any(m.is_async == exp.is_async for m in variants):
        ...
        scope_name_counts: Counter = Counter((m.scope, m.name) for m in found_methods)
        ...
        rep = FileReport(
            ...
            missing=missing, async_mismatches=async_mismatches, signature_mismatches=signature_mismatches,
            visibility_mismatches=visibility_mismatches, wrapper_mismatches=wrapper_mismatches,
            duplicates_in_file=duplicates_in_file, extras=extras
        )
        for err in parse_errors:
            rep.missing.append(f"[ParseError] {err}")
        reports[rel_path] = rep

    cross_dups: Dict[str, List[str]] = {}
    if include_cross_file_duplicates:
        for name, files in cross_counter.items():
            uniq = sorted(set(files))
            if len(uniq) >= 2:
                cross_dups[name] = uniq
...
def render_markdown(reports: Dict[str, FileReport], cross_dups: Dict[str, List[str]]) -> str:
    ...
    md = []
    md.append("# ClaudeDex Verifier Report (PLUS)")
    md.append("")
    md.append("## Summary")
    ...
    for key in sorted(reports.keys()):
        ...
        if r.missing:
            md.append(f"- **Missing** ({len(r.missing)}): " + ", ".join(r.missing))
        if r.async_mismatches:
            md.append(f"- **Async mismatches** ({len(r.async_mismatches)}): " + "; ".join(r.async_mismatches))
        if r.signature_mismatches:
            md.append(f"- **Signature mismatches** ({len(r.signature_mismatches)}):")
            for s in r.signature_mismatches[:20]:
                md.append(f"  - {s}")
            if len(r.signature_mismatches) > 20:
                md.append("  - ...")
        if r.visibility_mismatches:
            md.append(f"- **Visibility mismatches** ({len(r.visibility_mismatches)}):")
            for s in r.visibility_mismatches[:20]:
                md.append(f"  - {s}")
            if len(r.visibility_mismatches) > 20:
                md.append("  - ...")
        if r.wrapper_mismatches:
            md.append(f"- **Wrapper issues** ({len(r.wrapper_mismatches)}):")
            for s in r.wrapper_mismatches[:20]:
                md.append(f"  - {s}")
            if len(r.wrapper_mismatches) > 20:
                md.append("  - ...")
        if r.duplicates_in_file:
            md.append(f"- **Duplicates** ({len(r.duplicates_in_file)}): " + "; ".join(r.duplicates_in_file))
        if r.extras:
            md.append(f"- Extras (not in expected list): " + ", ".join(r.extras[:20]) + (" ..." if len(r.extras) > 20 else ""))
        # Sample
        if r.found_methods or r.found_classes:
            ...
            if sample_classes:
                md.append(f"- Classes: {sample_classes}")
            if sample_methods:
                md.append(f"- Methods: {sample_methods}")
        md.append("")
    return "\n".join(md)
...
def main():
    ...
    parser.add_argument("--docs", nargs="+", default=["key_methods.md"],
                        help="One or more Markdown docs that list expected key methods/classes per file")
    parser.add_argument("--structure", default=None,
                        help="Optional corrected file structure markdown to enumerate files to check")
    parser.add_argument("--markdown-out", default="verifier_report.md", help="Path to write Markdown report")
    parser.add_argument("--json-out", default="verifier_report.json", help="Path to write JSON report")
    parser.add_argument("--fail-on-missing", action="store_true", help="Exit non-zero if any expected item is missing")
    args = parser.parse_args()

    root = Path(args.root).resolve()

    # Aggregate expectations from multiple docs
